...
async function GetData5()
{
    ...
    let Container = document.createElement("div");
            ...
            {
                Res += ` <div class="col-3 area-data mb-5">
                <div class="bg-dark rounded-4 pt-1 ps-1 pe-1">
                    <i style="color:red" class="Child fa-solid fa-city fa-3x mb-2"></i>
                    <h2>${MyData["meals"][i]["strArea"]}</h2>
                    </div>
            </div>
                `
            }
            ...
            for (let index = 0; index < document.querySelectorAll(".area-data").length; index++)
{
    document.querySelectorAll(".area-data")[index].addEventListener("click" , function()
    {
        let Keyword = document.querySelectorAll(".area-data")[index].firstElementChild.firstElementChild.nextElementSibling.textContent;
        document.body.removeChild(document.body.querySelector(".container"));
        
        async function GetData()
        {
            let Data = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${Keyword}`);
            let MyData = await Data.json();
            console.log(MyData);
            ...
            for(let i = 0; i<MyData["meals"].length; ++i)
            {
                ...
                <div class="main-container area-data-2">
            ...
            for (let index = 0; index < document.querySelectorAll(".area-data-2").length; index++)
            {
                document.querySelectorAll(".area-data-2")[index].addEventListener("click" , function()
                {
                    let Keyword2 = document.querySelectorAll(".area-data-2")[index].firstElementChild.nextElementSibling.firstElementChild.textContent;
                    document.body.removeChild(document.body.querySelector(".container"));
                    let id;
                    for(let i = 0; i < MyData["meals"].length; ++i)
                    {
