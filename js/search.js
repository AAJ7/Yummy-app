...
{
    let SearchName = document.getElementsByName("name")[0].value; 
...
for (let index = 0; index < document.querySelectorAll(".index-data").length; index++)
{
    document.querySelectorAll(".index-data")[index].addEventListener("click" , function()
    {
        ...
        async function GetData3()
                            {
                                        let Data3 = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${SearchName}`);
...
{
    let SearchName = document.getElementsByName("letter")[0].value; 
    SearchName = SearchName.charAt(0).toUpperCase() + SearchName.slice(1);
